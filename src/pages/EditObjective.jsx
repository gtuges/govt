...
const EditObjective = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3000/api/objectives/${id}`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch objective details");
        return res.json();
      })
      .then((data) => {
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/objectives/${id}`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <div className="create-mission-container">
      <div className="page-header">
        <h2>Edit Strategic Objective</h2>
      </div>
